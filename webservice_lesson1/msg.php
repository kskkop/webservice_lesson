...
if (empty($productInfo)) {
    error_log('エラー発生：商品情報を取得できませんでした');
    header("Location:mypage.php"); //マイページへ
}
//$viewDataから相手のユーザーIDを取り出す
$dealUserIds[] = $viewData[0]['sale_user']; //売り手
$dealUserIds[] = $viewData[0]['buy_user']; //買い手
if (($key = array_search($_SESSION['user_id'], $dealUserIds)) !== false) {
    //array_search 指定した値を配列で検索し、見つかった場合に対応する最初のキーを返す
    //$dealUserIds 売り手、買い手どちらの情報も入っている
    //array_searchで$dealUserIdsがセッションのuser_idかどうか調べている
    unset($dealUserIds[$key]); //unset 指定した変数の割当を削除する 相手のユーザー情報だけ残る
}
$partnerUserId = array_shift($dealUserIds); //unsetで自分のユーザーIDを削除したので先頭相手のユーザーIDを変数に入れる

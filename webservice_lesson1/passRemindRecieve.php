    ...
    if (empty($err_msg)) {
        ...
        if (empty($err_msg)) {
            ...
            if (empty($err_msg)) {
                ...
                $pass = $auth_key; //生成したパスワード
                debug('$passのなかみ'.print_r($pass,true));
                //例外処理
                try {
                    $dbh = dbConnect();
                    //SQL文作成
...
ログインページ：http://localhost:8888/webservice_lesson1/login.php
再発行パスワード：{$pass}
...
EOT;
                        ...
                        debug('セッション削除:' . print_r($_SESSION, true));
